...
const AddKey = () => {
  ...
  const { DownloadAddKey, isDownloading, newKey, setNewKey } = useUtils();
        ...
        <VStack mt="1em">
          <FormControl>
            <Input
              ...
              w="full"
              value={Object.keys(newKey)[0]}
              onChange={(e) =>
                setNewKey({
                  [`${e.target.value}`]: newKey[Object.keys(newKey)[0]],
                })
              }
            />
            <FormHelperText fontSize="9pt">
              The key you want to add on your metadata files
            </FormHelperText>
          </FormControl>
          <FormControl>
            <Input
              id="newMetadataKeyValue"
              placeholder="New Metadata Key Value"
              w="full"
              value={newKey[Object.keys(newKey)[0]]}
              onChange={(e) =>
                setNewKey({ [`${Object.keys(newKey)[0]}`]: e.target.value })
              }
            />
            <FormHelperText fontSize="9pt">
              The value of the key you want to add on your metadata files
            </FormHelperText>
          </FormControl>
          <Text>
            &quot;{Object.keys(newKey)[0] || ""}&quot;: &quot;
            {newKey[Object.keys(newKey)[0]] || ""}&quot;,
          </Text>
        </VStack>
      )}
